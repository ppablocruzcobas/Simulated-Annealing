...
class Application(QApplication):
	def __init__(self, argv, update_func):
		...
		self._tlim = self._fig.gca().plot(self._x, self._lim, "-r",
		                                  label="Temp.")
	...
	def append(self, temperature, value):
		...
		self._tlim[0].set_xdata(asarray(self._x))
		self._tlim[0].set_ydata(asarray(self._lim))
		
		self._fig.gca().set_xlim(left=len(self._x) - 100, right=max(self._x),
		                         auto=True)
		self._fig.gca().set_ylim(top=max(max(self._lim), max(self._y)) + 0.25,
		                         bottom=min(self._y), auto=True)
